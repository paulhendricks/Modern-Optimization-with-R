...
ST=inputVariableSet("x1","x2", "x3", "x4") # same order of AR arima component
...
{ 
  ...
  for(i in I)
    { 
     F[i]=f(TS[i-1],TS[i-2], sin(TS[i-1]), cos(TS[i-1]))
...
f3 <- f2
f3[f3 < 0] <- 0
e3=maeres(outsamples-f3); cat("gp_adj out-of-sample MAE=", e3, "\n")
